...
public class ComputerPlayer {
    ...
    public int makePlay(List<Card> hand, int suit, int rank) {
        ...
        for (int i = 0; i < hand.size(); i++) {
            ...
            if (tempRank != 8) {
                // Check if last card was 8
                if (rank == 8) {
                    if (suit == tempSuit) {
                        play = tempId;
                    }
                } else if (suit == tempSuit || // Check for same suit/rank
                        rank == tempRank) {
                    play = tempId;
                }
            }
    ...
    public int chooseSuit(List<Card> hand) {
        ...
        if (numClubs > numDiamonds &&
            numClubs > numHearts &&
            numClubs > numSpades) {
            suit = 200;
        } else if (numHearts > numDiamonds &&
                   numHearts > numClubs   &&
                   numHearts > numSpades) {
            suit = 300;
        } else if (numSpades > numDiamonds &&
                   numSpades > numClubs &&
                   numSpades > numHearts) {
            suit = 400;
        }
